...
import Component01 from "./index"
import Component01a from "./component01a"
...
`mask`の中には`p`タグなどを使い、適当な文字列を入れてください。
...
そしてこれを、「`hover`した時に`top: 0`にする」CSSを書いていきます。
...
<div className={Styles.boxWrapper} style={{marginBottom: "220px"}}>
...
とのことです。もうちょっとかみ砕いて言うと、状態の変化をアニメーションとして表現することができる機能であるということです。今回で言う「状態の変化」とは、`top: 70%`から`top: 0`への変化ですね。
